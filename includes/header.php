...
if (file_exists($config_file_path)) {
    ...
    if (!empty($announcement['site_logo_path'])) { $site_logo_path = $announcement['site_logo_path']; }
...
<head>
  ...
  <style>
    ...
    .edit-logo-btn, .edit-announcement-btn, .edit-social-btn { position: absolute; top: -5px; left: -5px; z-index: 10; background: #0d6efd; border: none; border-radius: 50%; width: 24px; height: 24px; color: white; font-size: 12px; display: flex; align-items: center; justify-content: center; cursor: pointer; }
  </style>
</head>
<body>
...
<header>
  ...
  <nav class="nav-top navbar py-2">
    <div class="container-fluid custom-container d-flex align-items-center justify-content-between">
      ...
      <!-- مركز الإعلانات (في المنتصف) -->
      <div class="flex-grow-1 d-none d-lg-flex justify-content-center px-4">
        <?php 
        $current_time = date('Y-m-d H:i:s');
        $is_published = ($announcement['status'] ?? 'Draft') === 'Published';
        ...
        if ($is_published && $is_in_time || $is_admin): 
        ?>
          <div class="w-100 text-center <?php echo $is_admin ? 'editable-admin-border position-relative p-2' : ''; ?>" style="max-width: 500px;">
            <?php if ($is_admin): ?>
                ...
                <?php if (!$is_published || !$is_in_time): ?><span class="badge bg-warning position-absolute" style="top: -10px; right: 10px; font-size: 9px;">مخفي</span><?php endif; ?>
            <?php endif; ?>
      ...
      <div class="social-icons d-none d-lg-flex gap-3">
         <?php foreach (($announcement['social_links'] ?? []) as $s): ?>
  ...
  <nav id="main-header" class="navbar navbar-expand-lg py-3">
    <div class="container-fluid custom-container d-flex align-items-center justify-content-between">
      <div class="logo-container d-lg-none">
        <a class="navbar-brand" href="index.php"><img src="<?php echo $path_prefix . $site_logo_path; ?>" height="50"></a>
      </div>
      <div class="collapse navbar-collapse">
        <ul class="navbar-nav gap-3">
          <?php foreach ($menu_links as $link): ?>
            <li class="nav-item"><a class="nav-link <?php echo ($link['active'] ?? false) ? 'active' : ''; ?>" href="<?php echo $path_prefix . $link['url']; ?>"><?php echo $link['title']; ?></a></li>
          <?php endforeach; ?>
        </ul>
      </div>
  ...
  <div class="offcanvas offcanvas-end" id="offcanvasNavbar">
    <div class="offcanvas-header"><button type="button" class="btn-close" data-bs-dismiss="offcanvas"></button></div>
    <div class="offcanvas-body">
      <ul class="navbar-nav">
        <?php foreach ($menu_links as $link): ?>
          <li class="nav-item"><a class="nav-link" href="<?php echo $path_prefix . $link['url']; ?>"><?php echo $link['title']; ?></a></li>
        <?php endforeach; ?>
      </ul>
    </div>
  </div>
