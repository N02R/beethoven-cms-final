...
// يمكنك إنشاء مخصص لاحقاً مثل ArrivalModel أو الاعتماد على SiteModel مباشرة
...
class ArrivalController {
    public function index(string $lang = 'de'): void {
        ...
        // تحديد مسار الجذر للمشروع (بالتعديل حسب عمق المجلد الحالي)
        $root_path = realpath(__DIR__ . '/../../../');

        // 1. جلب بيانات الهيدر والفوتر والإعدادات العامة لكل الموقع
        $data = SiteModel::getGlobalData();

        // 2. جلب بيانات صفحة الوصول الخاصة (يمكن ربطها بـ Model خاص أو SiteModel)
        // مثال: $arrivalData = ArrivalModel::getArrivalData();
        $global_settings = SiteModel::getSettings();
        $arrival_data = isset($global_settings['arrival_page']) ? json_decode($global_settings['arrival_page'], true) : [];
