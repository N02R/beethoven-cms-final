...
class HealthController {
    public function index(string $lang = 'de'): void {
        ...
        // 2. جلب بيانات صفحة التأمين الصحي عبر المودل المخصص وتوفيرها بالتسميات المتوافقة
        $health_data_array = HealthModel::getHealthData();
        $data['health_page'] = $health_data_array;
        $data['health_data'] = $health_data_array; // لضمان التوافق التام مع الحقول داخل الـ Modals
        ...
        // 2. استدعاء ملف الـ View الخاص بالتأمين الصحي (health.php)
        ...
        if ($is_admin) {
            $modals_file = $root_path . '/src/Views/edu-services/includes/admin_health_modals.php';
