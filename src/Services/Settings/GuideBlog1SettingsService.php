...
class GuideBlog1SettingsService
{
    ...
    {
        if (!str_starts_with($action, 'update_guide_blog1_')) {
            return false;
        }

        // جلب البيانات الحالية المخزنة كـ JSON وفكها
        $guideData = isset($currentSettings['guide_blog1_page']) ? json_decode($currentSettings['guide_blog1_page'], true) : [];
        if (!is_array($guideData)) {
            $guideData = [];
        }

        $stmt = $pdo->prepare("INSERT INTO site_settings (setting_key, setting_value) VALUES ('guide_blog1_page', :v) ON DUPLICATE KEY UPDATE setting_value = :v_update");
        ...
        elseif ($action === 'update_guide_blog1_notes') {
            ...
            // إعادة ترتيب المصفوفة لتفادي الفجوات في الفهارس
        ...
        $stmt->execute(['v' => $jsonVal, 'v_update' => $jsonVal]);
