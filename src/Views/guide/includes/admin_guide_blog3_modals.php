<?php
// ملف مودلات لوحة التحكم الخاصة بصفحة Guide Blog 3
        ...
        <div class="modal-content">
            ...
            <div class="modal-body p-4">
                <form id="guideBreadcrumbForm" class="admin-settings-form">
                    <input type="hidden" name="action" value="update_guide_blog3_breadcrumb">
        ...
        <div class="modal-content">
            ...
            <div class="modal-body p-4">
                <form id="guideHeroForm" class="admin-settings-form" enctype="multipart/form-data">
                    <input type="hidden" name="action" value="update_guide_blog3_hero">
                    <input type="hidden" name="csrf_token" value="<?php echo htmlspecialchars($csrf_token ?? '', ENT_QUOTES, 'UTF-8'); ?>">
        ...
        <div class="modal-content">
            ...
            <div class="modal-body p-4">
                <form id="guideMainForm" class="admin-settings-form">
                    <input type="hidden" name="action" value="update_guide_blog3_main">
                    <input type="hidden" name="csrf_token" value="<?php echo htmlspecialchars($csrf_token ?? '', ENT_QUOTES, 'UTF-8'); ?>">
...
<!-- 4. Modal: تعديل الملاحظات / ميزات المعالجة (Notes) -->
<div class="modal fade custom-modal" id="guideNotesModal" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title"><i class="bi bi-journal-text text-primary"></i> تعديل قسم ميزات المعالجة (Notes)</h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
            </div>
            <div class="modal-body p-4">
                <form id="guideNotesForm" class="admin-settings-form">
                    <input type="hidden" name="action" value="update_guide_blog3_notes">
                    <input type="hidden" name="csrf_token" value="<?php echo htmlspecialchars($csrf_token ?? '', ENT_QUOTES, 'UTF-8'); ?>">
                    ...
                    <div class="p-4 shadow-sm mb-4" style="background: #ffffff; border-radius: 16px; border: 1px solid #e2e8f0;">
                        ...
                        <label class="small fw-bold mb-2 text-secondary">عناصر الميزات</label>
                                ...
                                <?php foreach ($guide_data['notes_items'] as $index => $item): ?>
                                    <div class="p-3 shadow-sm guide-note-row-item" id="note_row_<?php echo $index; ?>" style="background: #ffffff; border-radius: 16px; border: 1px solid #e2e8f0 !important;">
                                        <div class="row g-2 mb-3">
                                            <div class="col-12">
                                                <label class="form-label fw-semibold small text-secondary">عنوان العنصر (بخط عريض)</label>
                                                <input type="text" class="form-control guide-note-title" name="notes[<?php echo $index; ?>][title]" value="<?php echo htmlspecialchars($item['title'] ?? '', ENT_QUOTES, 'UTF-8'); ?>" placeholder="العنوان">
                                            </div>
                                            <div class="col-12">
                                                <label class="form-label fw-semibold small text-secondary">نص العنصر</label>
                                        ...
                                        <div class="text-end">
                                            <button type="button" class="btn-icon-trash mx-auto" onclick="removeRow('note_row_<?php echo $index; ?>')" title="حذف العنصر"><i class="bi bi-trash"></i></button>
                                        </div>
                                    </div>
                                <?php endforeach; ?>
                            <?php endif; ?>
                        ...
                        <div class="mt-3">
                            <button type="button" class="btn btn-outline-primary btn-sm w-100 py-2 rounded-3 fw-bold" onclick="addGuideNoteRow()">
                                <i class="bi bi-plus-circle me-1"></i> إضافة عنصر جديد
                            </button>
                        </div>
                    </div>
                </form>
...
<!-- 5. Modal: تعديل التايم لاين / مراحل الطلب (Timeline) -->
<div class="modal fade custom-modal" id="guideTimelineModal" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title"><i class="bi bi-list-ol text-primary"></i> تعديل مراحل معالجة الطلب (Timeline)</h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
            </div>
            <div class="modal-body p-4">
                <form id="guideTimelineForm" class="admin-settings-form">
                    <input type="hidden" name="action" value="update_guide_blog3_timeline">
                    <input type="hidden" name="csrf_token" value="<?php echo htmlspecialchars($csrf_token ?? '', ENT_QUOTES, 'UTF-8'); ?>">
                    ...
                    <div class="p-4 shadow-sm mb-4" style="background: #ffffff; border-radius: 16px; border: 1px solid #e2e8f0;">
                        ...
                        <label class="small fw-bold mb-2 text-secondary">المراجل / الخطوات</label>
                            ...
                            <?php if (!empty($guide_data['timeline_steps']) && is_array($guide_data['timeline_steps'])): ?>
                                <?php foreach ($guide_data['timeline_steps'] as $index => $step): ?>
                                    <div class="p-3 shadow-sm guide-timeline-row-item" id="guide_step_row_<?php echo $index; ?>" style="background: #ffffff; border-radius: 16px; border: 1px solid #e2e8f0 !important;">
                                        <div class="row g-2 mb-3">
                                            <div class="col-md-6">
                                                <label class="form-label fw-semibold small text-secondary">عنوان الخطوة الأساسي (مثل: الخطوة 01)</label>
                                            ...
                                            <div class="col-md-6">
                                                <label class="form-label fw-semibold small text-secondary">العنوان الفرعي للمرحلة</label>
                        ...
                        <div class="mt-3">
                            <button type="button" class="btn btn-outline-primary btn-sm w-100 py-2 rounded-3 fw-bold" onclick="addGuideTimelineRow()">
                                <i class="bi bi-plus-circle me-1"></i> إضافة مرحلة جديدة
                            </button>
                        </div>
                    </div>
                </form>
...
<script>
    ...
    function addGuideNoteRow() {
        ...
        div.innerHTML = `
            <div class="row g-2 mb-3">
                <div class="col-12">
                    <label class="form-label fw-semibold small text-secondary">عنوان العنصر (بخط عريض)</label>
                    <input type="text" class="form-control guide-note-title" name="notes[${count}][title]" placeholder="العنوان">
                </div>
                <div class="col-12">
                    <label class="form-label fw-semibold small text-secondary">نص العنصر</label>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        const forms = document.querySelectorAll('#guideNotesForm, #guideTimelineForm, #guideBreadcrumbForm, #guideHeroForm, #guideMainForm');
