    ...
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">
            ...
            <div class="modal-body p-4">
                <form id="jobAgrBreadcrumbForm" method="POST">
                    ...
                    <div class="mb-3">
                        ...
                        <input type="text" class="form-control" name="page_breadcrumb" value="<?php echo htmlspecialchars($job_agreements_data['page_breadcrumb'] ?? ''); ?>" required>
                    </div>
                    <div class="mb-3">
                        <label class="form-label fw-bold">رابط المسار الأخير</label>
                        <input type="text" class="form-control" name="page_breadcrumb_url" value="<?php echo htmlspecialchars($job_agreements_data['page_breadcrumb_url'] ?? '#'); ?>">
                    </div>
                </form>
            </div>
...
<div class="modal fade custom-modal" id="jobAgrHeroModal" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered modal-lg">
        <div class="modal-content">
            ...
            <div class="modal-body p-4">
                <form id="jobAgrHeroForm" method="POST" enctype="multipart/form-data">
                    ...
                    <?php if (!empty($job_agreements_data['hero_img'])): ?>
                        <div class="mb-3 p-2 border rounded bg-light text-center">
                            <img src="<?php echo $path_prefix . htmlspecialchars($job_agreements_data['hero_img']); ?>" style="max-height: 120px; object-fit: contain;" alt="Hero Preview">
                        </div>
                    <?php endif; ?>
                    <input type="hidden" name="old_img" value="<?php echo htmlspecialchars($job_agreements_data['hero_img'] ?? ''); ?>">
                    <div class="mb-3">
                    ...
                    <div class="mb-3">
                        ...
                        <input type="text" class="form-control" name="hero_position" value="<?php echo htmlspecialchars($job_agreements_data['hero_position'] ?? 'center center'); ?>">
                    </div>
                </form>
            </div>
            <div class="modal-footer">
...
<div class="modal fade custom-modal" id="jobAgrMainModal" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered modal-lg">
        <div class="modal-content">
            ...
            <div class="modal-body p-4">
                <form id="jobAgrMainForm" method="POST">
                    <input type="hidden" name="action" value="update_job_agreements_main">
                    <div class="mb-3">
                        <label class="form-label fw-bold">العنوان الرئيسي</label>
                        <input type="text" class="form-control" name="main_title" value="<?php echo htmlspecialchars($job_agreements_data['main_title'] ?? ''); ?>" required>
                    </div>
                    <div class="mb-3">
                        <label class="form-label fw-bold">الوصف التفصيلي</label>
                        <textarea class="form-control" name="main_desc" rows="5" required><?php echo htmlspecialchars($job_agreements_data['main_desc'] ?? ''); ?></textarea>
                    </div>
                </form>
            </div>
            <div class="modal-footer">
...
<div class="modal fade custom-modal" id="jobAgrNotesModal" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered modal-lg">
        <div class="modal-content">
            ...
            <div class="modal-body p-4">
                <form id="jobAgrNotesForm" method="POST">
                    <input type="hidden" name="action" value="update_job_agreements_notes">
                    <div class="mb-3">
                        <label class="form-label fw-bold">نص الملاحظة</label>
                        <textarea class="form-control" name="note_text" rows="3" required><?php echo htmlspecialchars($job_agreements_data['note_text'] ?? ''); ?></textarea>
                    </div>
                </form>
            </div>
            <div class="modal-footer">
...
<div class="modal fade custom-modal" id="jobAgrCardModal" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered modal-lg">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title"><i class="bi bi-file-earmark-arrow-down text-primary"></i> تعديل ملف الاتفاقية</h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
            </div>
            <div class="modal-body p-4">
                <form id="jobAgrCardForm" method="POST" enctype="multipart/form-data">
                    <input type="hidden" name="action" value="update_job_agreements_card">
                    <input type="hidden" name="old_file" value="<?php echo htmlspecialchars($job_agreements_data['download_item']['file'] ?? ''); ?>">

                    <div class="mb-3">
                        <label class="form-label fw-bold">عنوان الملف الظاهر</label>
                        <input type="text" class="form-control" name="item_title" value="<?php echo htmlspecialchars($job_agreements_data['download_item']['title'] ?? ''); ?>" required>
                    </div>

                    <div class="mb-3">
                        <label class="form-label fw-bold">النص الفرعي (Sub)</label>
                        <input type="text" class="form-control" name="item_sub" value="<?php echo htmlspecialchars($job_agreements_data['download_item']['sub'] ?? ''); ?>">
                    </div>

                    <div class="mb-3">
                        <label class="form-label fw-bold">نوع الملف</label>
                        <select class="form-select" name="item_type">
                            <option value="pdf" <?php echo (($job_agreements_data['download_item']['type'] ?? '') === 'pdf') ? 'selected' : ''; ?>>ملف PDF</option>
                            <option value="word" <?php echo (in_array($job_agreements_data['download_item']['type'] ?? '', ['word', 'docx'])) ? 'selected' : ''; ?>>ملف Word (docx)</option>
                        </select>
                    </div>
                    ...
                    <div class="mb-3">
                        ...
                        <div class="form-text text-muted mt-1">الملف الحالي: <?php echo htmlspecialchars($job_agreements_data['download_item']['file'] ?? 'لا يوجد'); ?></div>
                    </div>
                </form>
            </div>
            <div class="modal-footer">
...
<!-- JavaScript Engine -->
<script>
    document.querySelectorAll('#jobAgrBreadcrumbForm, #jobAgrHeroForm, #jobAgrMainForm, #jobAgrNotesForm, #jobAgrCardForm').forEach(form => {
